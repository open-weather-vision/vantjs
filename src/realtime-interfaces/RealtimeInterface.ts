 ...
 */
export default abstract class RealtimeInterface<
    Interface extends WeatherStation | WeatherStationAdvanced
> extends TypedEmitter<RealtimeInterfaceEvents> {
    ...
    /**
     * Destroys the realtime interface, fires a `"destroy"` event. The update cycle is stopped. If
     * the interface has been created using `.connect(settings)` the connection to the device gets closed.
